...
static bool
create_tooltip_for_action(const Glib::RefPtr<Gtk::Tooltip>& tooltip_, const std::string& action_name, const std::string& tooltip) {
	auto accels = App::instance()->get_accels_for_action(action_name);
	std::string accels_txt;
	for (const auto& accel : accels) {
		accels_txt += _("\nAccel: ") + accel;
	}
	tooltip_->set_text(tooltip + accels_txt);
	return true;
}
...
template<class T>
void
set_button_tooltip(T& button, const std::string& action_name, const std::string& tooltip)
{
	button.property_has_tooltip() = true;
	button.signal_query_tooltip().connect(
		sigc::bind(
			sigc::bind(
				sigc::hide<0>(sigc::hide<0>(sigc::hide<0>( // remove the first 3 args
					sigc::ptr_fun(create_tooltip_for_action)))),
				tooltip
			),
			action_name
		)
...
ActionWidgetHelper::init_toolbutton(Gtk::ToolButton& button, const std::string& action_name, const std::string& icon_name, const std::string& label, const std::string& tooltip)
{
	...
	set_button_tooltip(button, action_name, tooltip);
	button.show();
